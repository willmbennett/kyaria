...
const handleResumeImport = <T extends sectionType>(id: string, section?: sectionType[]): T[] => {
...
export const transformDataToFormValues = (data: Partial<ResumeClass>): ResumeBuilderFormData => {
    const {
        name,
        title,
        email,
    ...
    const defaultSkills = skills ? skills.map(skill => ({ label: skill, value: skill })) : [];
    const defaultInterests = interests ? interests.map(interest => ({ label: interest, value: interest })) : [];

    const socialLinksArray = social_links ? Object.entries(social_links).map(([name, url]) => ({ name, url })) : [];
    ...
    return {
        ...
        title: title || '',
        name: name || '',
        education: handleResumeImport<EducationFormData>('education', education),
        professional_experience: handleResumeImport<ProfessionalExperienceFormData>('professional_experience', professional_experience),
        skills: defaultSkills,
        projects: handleResumeImport<ProjectFormData>('projects', projects),
        interests: defaultInterests,
        social_links: socialLinksArray,
        certifications: handleResumeImport<CertificationFormData>('certifications', certifications),
        publications: handleResumeImport<PublicationFormData>('publications', publications),
        awards: handleResumeImport<AwardFormData>('awards', awards),
        volunteering: handleResumeImport<VolunteeringFormData>('volunteering', volunteering),
...
export function convertFormDataToResumeModel(formData: any, originalData: any): any {
    ...
    type OriginalDataKey = keyof typeof originalData;
    type FormDataKey = keyof ResumeBuilderFormData;

    (Object.keys(formData) as Array<FormDataKey>).forEach(key => {
        const formDataValue = formData[key];
        if (key in originalData) {
            const originalDataKey = key as OriginalDataKey;
            const originalDataValue = originalData[originalDataKey];

            switch (key) {
                case 'social_links':
                    updatedData.social_links = formData.social_links?.reduce((acc: SocialLinksMap, link: any) => {
                        acc[link.name] = link.url;
                        return acc;
                    }, {}) || originalData.social_links;
                    break;
                case 'skills':
                    updatedData.skills = formData.skills?.map((skill: any) => skill.label) || originalData.skills;
                    break;
                case 'interests':
                    updatedData.interests = formData.interests?.map((interest: any) => interest.label) || originalData.interests;
                    break;
                default:
                    if (Array.isArray(formDataValue)) {
                        if (formDataValue.length > 0 && typeof formDataValue[0] === 'object') {
                            if (Array.isArray(originalDataValue)) {
                                // Map each item in the array, assuming they are objects
                                updatedData[originalDataKey] = formDataValue.map((item, index) =>
                                    convertFormDataToResumeModel(item, originalDataValue[index] || {})
                                );
                            } else {
                                // Directly assign the formDataValue when originalDataValue is not an array
                                updatedData[originalDataKey] = formDataValue;
                            }
                            break;
                        } else {
                            // Direct assignment for arrays of primitive types
                            updatedData[originalDataKey] = formDataValue;
                            break;
                        }
                    } else if (typeof formDataValue === 'object' && formDataValue !== null && typeof originalDataValue === 'object') {
                        // Handle nested objects
                        updatedData[originalDataKey] = convertFormDataToResumeModel(formDataValue, originalDataValue || {});
                    } else {
            ...
            // If the key is not in originalData, add it to updatedData
            updatedData[key] = formDataValue;
        }
    });

    return updatedData as ResumeClass;
}
    ...
    if (sourceData.ContactInformation) {
        ...
        if (sourceData.ContactInformation.WebAddresses) {
            sourceData.ContactInformation.WebAddresses.forEach(webAddress => {
                transformedData.social_links ? transformedData.social_links[webAddress.Type] = webAddress.Address : null;
            });
        }
        // Location
...
export interface ResumeBuilderFormData {
    ...
    location?: string;
    summary?: string;
    professional_experience?: ProfessionalExperienceFormData[];
    education?: EducationFormData[];
    projects?: ProjectFormData[];
    publications?: PublicationFormData[];
    volunteering?: VolunteeringFormData[];
...
interface ProfessionalExperienceFormData {
    title?: string;
    company?: string;
    location?: string;
    start_date?: string;
    end_date?: string;
    responsibilities: { content: string }[];
}
