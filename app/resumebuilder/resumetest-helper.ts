...
export function convertFormDataToResumeModel(formData: ResumeBuilderFormData): Partial<ResumeClass> {

    const socialLinksMap: SocialLinksMap = formData.social_links?.reduce((acc, link) => {
        acc[link.name] = link.url;
        return acc;
    }, {} as SocialLinksMap) || {};

    // Create an object that matches the structure of ResumeClass
    const resumeData = {
        name: formData.name || '',
